  ...
  if(millis() - previousDataSend > telemetrySendInterval){
    ...
    if (updates) {
      ...
      if (utctime != lastutc) {
        ...
        Serial.print("UTC:");
        tb.sendAttributeData("utc",utctime);
        Serial.print(utctime);
        Serial.print(" LAT:");
        lat = (float)lat / 1000000, 6;
        tb.sendAttributeData("lat",lat );
        Serial.print((float)lat / 1000000, 6);
        Serial.print(" LNG:");
        lng = (float)lng / 1000000, 6;
        tb.sendTelemetryData("lng",lng);
        Serial.print((float)lng / 1000000, 6);
        Serial.print(" Speed:");
        tb.sendTelemetryData("speed",speed);
        Serial.print(speed);
        Serial.print("km/h Alt:");
        tb.sendTelemetryData("km/h_alt", alt);
        Serial.print(alt);
        Serial.print("m Sats:");
        tb.sendTelemetryData("m_sats", sats);
        Serial.println(sats);
        tb.sendTelemetryData("teste_update", random(0,101));
      #if USE_SOFT_SERIAL
    ...
    tb.sendTelemetryData("teste_telemetria", random(0,101));
  }
  tb.loop();
  delay(50);
}
